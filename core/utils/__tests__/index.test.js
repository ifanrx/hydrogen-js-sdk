...
describe('utils', () => {
  it('#log', () => {
    expect(utils.log).toEqual(expect.any(Function))
  })
  ...
  it('#format', () => {
    ...
    expect(utils.format(url, params)).toBe('/hserve/v1/table/10/')
    ...
    expect(utils.format(url, params)).toBe('/hserve/v1/table/10/record/20/')
  ...
  it('#parseRegExp', () => {
    ...
    expect(result).toEqual([regExpString, 'gi'])
  })
  ...
  it('replaceQueryParams', () => {
    ...
    expect(result).toEqual({'category_id': 12, otherData: 100})
  })
  ...
  it('compareVersion', () => {
    ...
    for (let i = 0; i < versions.length; i++) {
      expect(utils.compareVersion(versions[i][0], versions[i][1])).toEqual(versions[i][2])
    }
  })
  ...
  it('#doCreateRequestMethod()', () => {
    ...
    return BaaS.fakeFunction({
      ...
      expect(BaaS._baasRequest.mock.calls[0][0]).toEqual({
