...
const ticketReportThrottle = ticketReportFn => (formID, {enableThrottle = false} = {}) => {
  ...
  // 上次调用时间与当前时刻对比，未超过 5s 则调用失败
  if (lastInvokeTime && now - lastInvokeTime <= TICKET_REPORT_INVOKE_LIMIT.MIN_INTERVAL_PRE_TIME) return Promise.resolve()

  return storageAsync.get(STORAGE_KEY.REPORT_TICKET_INVOKE_RECORD).then(invokeRecord => {
    ...
    // 当调用次数超过 10 次，且第一次调用时间距离此刻未超过 24h，则调用失败
    if (invokeRecord && invokeRecord.invokeTimes >= TICKET_REPORT_INVOKE_LIMIT.TIMES_LIMIT.MAX_TIMES_PER_CYCLE && !isOverdue) return Promise.resolve()
