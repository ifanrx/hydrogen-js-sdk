...
const createThirdPartyAuthFn = BaaS => () => {
  ...
  const sourceWindow = iframe === 'true' ? window.parent : window.opener
  if (accessToken) {
...
// login
const createLoginWithThirdPartyFn = BaaS => (provider, {
  createUser = true,
  syncUserProfile,
  authPageUrl,
  iframe = false,
} = {}) => getThirdPartyAuthToken({authPageUrl, provider, iframe})
  .then(token => {
